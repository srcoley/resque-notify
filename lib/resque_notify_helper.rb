...
module ResqueNotifyHelper
  class << self
    ...
    def enqueue_job(args)
      Resque.redis.lpush("queue:#{args[:queue]}", ResqueNotifyHelper.encode(:class => args[:class].to_s, :args => args[:args]))
    end

    def check_paused(args)
      if ResqueNotifyHelper.paused?(args[:queue])
        enqueue_job(args)
